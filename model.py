...
class DQN:
    ...
    def _build_network(self, name):
        with tf.variable_scope(name):
            model = tf.layers.conv2d(inputs=self.input_X, filters=32, kernel_size=[4, 4], padding='same', activation=tf.nn.relu)
            model = tf.layers.conv2d(inputs=model, filters=64, kernel_size=[2, 2], padding='same', activation=tf.nn.relu)
            #model = tf.layers.max_pooling2d(inputs=model, pool_size=[2, 2], strides=2, padding='same')
            model = tf.layers.conv2d(inputs=model, filters=128, kernel_size=[2, 2], padding='same', activation=tf.nn.relu)
            #model = tf.layers.max_pooling2d(inputs=model, pool_size=[2, 2], strides=2, padding='same')
            model = tf.layers.conv2d(inputs=model, filters=256, kernel_size=[2, 2], padding='same', activation=tf.nn.relu)
            #model = tf.layers.max_pooling2d(inputs=model, pool_size=[2, 2], strides=2, padding='same')
            model = tf.contrib.layers.flatten(inputs=model)
